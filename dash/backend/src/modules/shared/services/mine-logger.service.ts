    ...
    try {
      ...
      if (
        ...
        this.mailerService.sendMail({
          ...
          this.log({
            label: 'Error notification email sent.',
            data: { mailSendResponse: errorEmailResponse }
          }, MineLoggerService.name);
        }).catch(e => {
          this.log({
            label: 'Failed to send error notification email',
            data: { mailSendResponse: e.message }
          }, MineLoggerService.name);
        });
      }
    } catch (e) {
